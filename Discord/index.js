...
client.on("ready", () => {
  ...
  renamerFunction = setInterval(function () {
    renamer();
  }, 300000);
  loadSongs();
});
...
client.on("message", async (message) => {
  ...
  if (command === "play") {
    if (message.member.voice.channel) {
      ...
      if (bassboost) volume *= 5000;

      playSong(connection, currentSong, volume);
  ...
  if (command === "loop") {
    isLooped = !isLooped;
    var msg;
    if (isLooped) msg = "is looped";
    else msg = "is not looped";
    message.reply("Song is " + msg);
  }

  if (command === "spam") {
    spammessage = args.join(" ");
    spamFunction = setInterval(function () {
      spammer();
    }, 1000);
  }
...
function playSong(connection, currentSong, volume) {
